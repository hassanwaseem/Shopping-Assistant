(function (root, factory) {
  ...
  const DESSERT_SIGNAL = /\b(ice[ -]?cream|sundae|halwa|panjeeri|pinjiri|kheer|kulfi|gulab jamun|jalebi|seviyan|sheer khurma|mithai|pudding|zarda|cake|cupcake|rusk|biscuit|cookie|brownie|muffins?|banana bread|trifle|tiramisu|mousse|custard|donut|doughnut|falooda|flan|rabri|barfi|laddu|ladoo|rasmalai|baklava|kunafa|nutella|buttercream|apple puff pastry tart|almond pastry|black forest pastry|chocolate cinnamon rolls|chocolate croissants?|chocolate date bites|chocolate lasagna|caramel shahi tukray)\b/;
  ...
  const COLLECTION_SIGNAL = /\b(\d+[ -]*(?:ways|uses|recipes|home remedies)|\d+\s+(?:\w+\s+){1,3}recipes?|ways to|recipe collection|recipes from leftover|lunch box ideas?)\b/;
  const COMPONENT_SIGNAL = /\b(spice mix|masala powder|cooking sauce|dipping sauce|seasoning|marinade|premix|home remedies?|how to cook|how to make|how to store|cleaning method|homemade .* masala|homemade khoya|garam masala recipe|chaat masala recipe|kunafa dough|kataifi pastry|pizza sauce|pizza crust|sauce recipe|buttercream|powder for (?:kids|babies))\b/;
  const STANDALONE_COMPONENT_SIGNAL = /^(?:homemade\s+)?(?:.+\s+)?(?:paste|stock|syrup|jam|chutney|sauce|dough|khoya|peanut butter|powder|seasoning|marinade|premix|breadcrumbs?|mayonnaise|pickle|puree|condensed milk|mascarpone cheese|ricotta cheese|puff pastry|butter and ghee|dahi\/?yogurt|chicken spread|talbina mix|kheer mix)(?:\s+recipe)?(?:\s*\([^)]*\))?$/;
